 ...
 * that context. Allows for execution in process (synchronous) as well as in
 * background thread (asynchronous).
 * 
 * @author Andrew Bowley 25 Mar 2015
 */
public class JavaPersistenceContext {
	...
	/**
	 * JPA EntityManager "lite" provider hides special case where a reserved
	 * ConnectionSource must be employed
	 */
	 ...
	 * 
	 * @param persistenceWork       Work to be performed in Java
	 *                              PersistenceUnitAdmin context
	...
	public Boolean doTask() {
		...
		// Now set actual enclosing transaction in transactionInfo object so it is
		// available on commit
		if (transactionInfo.isUserTransaction()) {
			...
			try { // Use container managed transaction. User can only request rollback.
				if (userTransactionSupport != null)
		...
		// Commence work, ready to catch RuntimeExceptions declared to be throwable by
		// EntityManager API
		...
		// Other runtime exceptions are captured by the WorkerTask and reported
		// onExecuteComplete()
		finally {
			setRollbackOnly = resolveOutcome(entityManager, userTransactionSupport, transactionInfo, success,
					rollbackException);
		}
	...
	public void onPostExecute(Boolean success) {
		...
		if (executionException != null) {
			...
			// If rollback exception not already captured, set the uncaught exception as the
			// rollback cause
		...
		if (rollbackException != null) {
			...
			logger.error("PersistenceUnitAdmin container rolled back transaction", rollbackException);
		} else
	...
	protected WorkStatus getStatus() {
		return status;
	}

	/**
	 * Resolve outcome of persistence work given all relevant parameters. Includes
	 * Commit/rollback by calling EntityManager close(), unless unexpected exception
	 * has occurred.
	 * 
	 ...
	 * @param rollbackException      Optional RuntimeException thrown by
	 *                               EntityManager object
	 * @return boolean - If rollback, then true
	 */
	protected boolean resolveOutcome(EntityManagerLite entityManager, UserTransactionSupport userTransactionSupport,
			TransactionInfo transactionInfo, boolean success, Throwable rollbackException) {
		...
		if (userTransactionSupport != null)
			userTransactionSupport.setUserTransaction(true);
		EntityTransaction transaction = entityManager.getTransaction();
		if (!success && (rollbackException == null)) { // Unexpected exception thrown. Just rollback.
			if (transaction.isActive())
				transaction.rollback();
		} else {
			if (rollbackException != null) { // RuntimeException caught, so do rollback
			...
			try {
				...
				entityManager.close();
			} // PersistenceUnitAdmin exception may be thrown on commit or rollback. Just log
				// it.
			catch (PersistenceException e) { // Ensure onRollback() is called on PersistenceWork object
				if (rollbackException == null)
					transactionInfo.setRollbackException(e);
				setRollbackOnly = true;
				logger.error("PersistenceUnitAdmin error on commit", e);
			}
		}
