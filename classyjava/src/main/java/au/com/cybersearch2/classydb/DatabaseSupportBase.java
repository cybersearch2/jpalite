...
public abstract class DatabaseSupportBase implements DatabaseSupport, ConnectionSourceFactory 
{
	...
	public void setVersion(int version, Properties properties, ConnectionSource connectionSource) 
	{
		...
		try 
		{
			...
			tableExists = connection.isTableExists(infoTableName);
			if (tableExists)
				connection.executeStatement(getVersionUpdateStatement(infoTableName, version), DatabaseConnection.DEFAULT_RESULT_FLAGS);
			else
			{
				connection.executeStatement(getVersionCreateStatement(infoTableName), DatabaseConnection.DEFAULT_RESULT_FLAGS);
				connection.executeStatement(getVersionInsertStatement(infoTableName, version), DatabaseConnection.DEFAULT_RESULT_FLAGS);
			}
		} 
		catch (SQLException e) 
		{
